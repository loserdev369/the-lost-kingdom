...
// todo - add ability for admin the gardener the mint a specific number of nft to donate to charity
// todo - create test to make sure that this works
// todo - add getters and setters for important variables even though i don't think they will need a price setter
...
contract DesperateApeWives is Ownable, ERC721Enumerable, PaymentSplitter {
    ...
    uint private walletLimit = 3; // updated with sale stages
    ...
    uint public presaleEnd = 1635303600;   // October 27th 11PM EST
        ...
        PaymentSplitter(team, team_shares)
    {
        ...
        _reserved = 190; // this is for production
        ...
        // Todo - ONLY FOR DEBUGGING update for production
        presaleStart = block.timestamp + 5 minutes;
        presaleEnd = presaleStart + 5 minutes;
        publicSale = presaleEnd + 5 minutes;
    }
    ...
    function resetSaleTimes() external onlyAdmin {
        uint addTime = 3 minutes;
        presaleStart = block.timestamp + 1 minutes;
        presaleEnd = presaleStart + addTime;
        publicSale = presaleEnd + addTime;
    }
    ...
    function adminMint() external onlyAdmin {
        ...
        console.log("totalSupply + amount", totalSupply() + _amount);
        console.log("MAX_WIVES", MAX_WIVES);
